/**
 * hux's flexbox layout engine.
 *
 * Single-pass CSS-flexbox implementation sized for terminal rendering:
 * every pane, tab, status bar, drag ghost, and overlay in the UI is laid out
 * through a `FlexNode` tree whose leaves are pre-sized cell grids. Inputs
 * are style values (width/height/min/max, flex grow/shrink, justify/align,
 * padding/border/gap); outputs are integer-rounded cell coordinates on each
 * node's `.layout`.
 *
 * Supported style surface (see `FlexNode` setters):
 ...
 *
 * What's intentionally out of scope: absolute positioning, margins, wrap,
 * baseline alignment, aspect-ratio, box-sizing, RTL. If a feature isn't
 * reachable through a setter here, don't add it — hux doesn't need it.
 *
 * Lineage: descended from Meta's Yoga (github.com/facebook/yoga, MIT), but
 * heavily trimmed and tuned for hux's re-layout patterns — the multi-entry
 * caches on `FlexNode` (`_lW`/`_cIn`/`_fbBasis`) are the hot path that
 * makes drag and scrollback cheap to redraw.
 */
...
export { Align, Direction, Edge, FlexDirection, Gutter, Justify }
...
// Fast-path flag helper for _hasPadding / _hasBorder. Unit.Undefined = 0.
...
export class FlexNode {
  ...
  calculateLayout(
    ...
    // Terminal cells are integer units — hardcode the point-scale to 1 so we
    // don't need a per-node Config object just to carry the constant around.
    roundLayout(this, 1, 0, 0)
  }
}
...
function commitCacheOutputs(node: FlexNode, performLayout: boolean): void {
  if (performLayout) {
...
// Generation counter — bumped per calculateLayout. Nodes stamp _fbGen/_cGen
// when their cache is written; a cache entry with gen === _generation was
...
function layoutNode(
  ...
  // Leaf node (no children). Measure-func leaves from upstream Yoga — the
  // text-sizing hook that Ink used — aren't wired up here: hux lays out
  // pre-sized cell grids, so there's nothing to measure.
  ...
  // Gather flow children. display:contents nodes are transparent — their
  // children are lifted into the grandparent's child list (recursively), and
  // the contents node itself gets zero layout. (Absolute positioning isn't
  // wired up — no setPositionType — so every non-None child goes into flow.)
  const flowChildren: FlexNode[] = []
  collectLayoutChildren(node, flowChildren)
...
function computeFlexBasis(
  ...
  // Upstream yoga would pass availableMain with mode AtMost here when a
  // measure-func subtree is present, to keep text nodes from reporting
  // unconstrained intrinsic width as flex-basis. With measure-funcs gone,
  // main stays Undefined and pure-layout flex-grow children stay at basis 0
  // (matching YGMinMaxDimensionTest flex_grow_in_at_most).
  const mw = isMainRow ? NaN : crossConstraint
  const mh = isMainRow ? crossConstraint : NaN
  const mwMode = isMainRow ? MeasureMode.Undefined : crossConstraintMode
  const mhMode = isMainRow ? crossConstraintMode : MeasureMode.Undefined
...
function isStretchAlign(child: FlexNode): boolean {
...
function resolveChildAlign(parent: FlexNode, child: FlexNode): Align {
...
function calculateBaseline(node: FlexNode): number {
  let baselineChild: FlexNode | null = null
...
function isBaselineLayout(node: FlexNode, flowChildren: FlexNode[]): boolean {
...
function zeroLayoutRecursive(node: FlexNode): void {
  for (const c of node.children) {
    c.layout.left = 0
    c.layout.top = 0
    c.layout.width = 0
...
function collectLayoutChildren(node: FlexNode, flow: FlexNode[]): void {
  // Gather flow children, flattening display:contents subtrees so their
  // children are laid out as direct children of this node (per CSS
  // display:contents — the box is removed from the layout tree but its
  // children remain, lifted to the grandparent). display:none zeros out the
  // subtree and drops it. Nothing routes to an absolute list anymore —
  // setPositionType was pruned from the public API.
  for (const c of node.children) {
...
function roundLayout(
  ...
  // Upstream YGRoundValueToPixelGrid rounds text leaves (measureFunc nodes)
  // differently — floor positions and ceil fractional widths to keep glyphs
  // from clipping. With measureFunc pruned, every node is non-text, so we
  // use standard rounding throughout. Width/height go through absolute edges
  // to avoid cumulative drift.
  l.left = roundValue(nodeLeft, scale)
  l.top = roundValue(nodeTop, scale)

  const absRight = absNodeLeft + nodeWidth
  const absBottom = absNodeTop + nodeHeight
  l.width = roundValue(absRight, scale) - roundValue(absNodeLeft, scale)
  l.height = roundValue(absBottom, scale) - roundValue(absNodeTop, scale)
...
function roundValue(v: number, scale: number): number {
  let scaled = v * scale
  let frac = scaled - Math.floor(scaled)
  if (frac < 0) frac += 1
  // Float-epsilon tolerance matches upstream YGDoubleEqual (1e-4). Upstream's
  // forceCeil/forceFloor branches (used for text leaves) are gone — hux has no
  // measure-func nodes, so everything rounds half-up.
  if (frac < 0.0001) return Math.floor(scaled) / scale
  if (frac > 0.9999) return Math.ceil(scaled) / scale
  return (Math.floor(scaled) + (frac >= 0.4999 ? 1 : 0)) / scale
}
